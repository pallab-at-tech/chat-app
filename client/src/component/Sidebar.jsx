...
const Sidebar = () => {
  ...
  return (
    <div className='h-full w-full grid grid-cols-[48px_1fr] bg-white'>
      <div className='bg-slate-100 w-12 h-full rounded-tr-md rounded-br-lg py-5 text-slate-500 flex flex-col justify-between' >
        <div>
            ...
            allUser.map((conv, index) => {
              return (
                <NavLink to={"/" + conv?.userDetails?._id} key={conv?._id} className='flex items-center gap-2 py-3 px-2 border border-transparent hover:border-primary rounded hover:bg-slate-100 cursor-pointer'>
                  ...
                  <div>
                    <h3 className='text-center line-clamp-1 font-semibold text-base'>{conv?.userDetails?.name}</h3>
                  ...
                  </div>
                  {
                    Boolean(conv?.unseenMsg) && (
                      <p className='text-xs w-6 h-6 flex justify-center items-center ml-auto p-1 bg-primary font-semibold rounded-full'>{conv?.unseenMsg}</p>
                    )
                  }
                </NavLink>
              )
